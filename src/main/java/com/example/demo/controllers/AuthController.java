...
public class AuthController {
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
		...
		if (strRoles == null && (!user.getUsername().equals("admin") || !user.getUsername().equals("barbero"))) {
			Rol userRole = roleRepository.findByName(ERol.ROLE_USER)
					.orElseThrow(() -> new RuntimeException("Error: Rol no encontrado."));
			roles.add(userRole);
		} else {
			if (strRoles == null && user.getUsername().equals("admin")) {
				Rol adminRole = roleRepository.findByName(ERol.ROLE_ADMIN)
						.orElseThrow(() -> new RuntimeException("Error: Rol no encontrado."));
				roles.add(adminRole);
			} else{
				if (strRoles == null && user.getUsername().equals("barbero")){
